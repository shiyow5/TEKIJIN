...
⛔ **このスクリプトが出した結論は取り下げてある**（`docs/benchmarks/c2.md` の断り書き）。
リクエストを手で写していて製品と違い、異常系20件を全部 C2 の担当として数えていた。
製品のまま測るには `research_faithful.py` + `research_c2_faithful.py` を使うこと（#113）。
ここは**当時の測定手順の記録**として残している。
