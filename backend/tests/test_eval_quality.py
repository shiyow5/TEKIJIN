...
def test_enough_independent_samples(person):
    ...
    上限は fixtures の構造で決まる。#51 で支援部署のメンバーを入れて 26 → 35 まで増やしたが、
    **一次データで案件を持つのは顧客接点のある4部署（16名）だけ**で、
    残り24名は案件のリードを一度も務めない。ここを超えるには一次データ側の変更が要る。
    """
    uniq = {tuple(sorted(q["gold_experts"])) for q in person}
    assert len(uniq) >= 30, f"独立サンプルが {len(uniq)} 種類しかない"
