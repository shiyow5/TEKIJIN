...
def test_enough_independent_samples(person):
    ...
    しきい値の変遷: 20 → 30 → **40**（#54）。#51 の時点では 35 しか無く、
    「一次データで案件を持つのは4部署16名だけ」という上限が効いていた。その後
    評価セットが 87件へ育ったことで、**一次データを触らないまま 50 に到達している**
    ので、DoD が求めていた 40 へ引き上げる（実測 50 なので余裕 10）。
