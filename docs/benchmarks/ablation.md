...
| | 層2 R@3 | MRR |
|---|---|---|
| 現行（Dense 検索 → チャンクを人へ集約） | 0.607 | 0.721 |
| **トピック媒介（LLM分類 → 構造化スコアラー）** | **0.747 – 0.771** | 0.836 |

**分割検証（56件を半分に割り、片方で構成を選び、もう片方で測る／200回）で
+0.118（中央値 +0.125、5–95%tile: +0.018 – +0.202、96.5% の分割で改善）。**
全件で最良構成を選んだときの +0.164 は多重比較で楽観的なので、採用判断はこの +0.118 で行う。
...
| 手法 | 効果 | 備考 |
|---|---|---|
| **トピック媒介の構造化スコアリング** | **+0.152** [+0.054,+0.247] | 下記 §4 |
| 検索文脈つきの LLM 分類（RAG型分類） | 段A acc@1 0.714 → **0.821** | +0.06s |
| 棄却は「証拠の十分性」を別段で判定 | L4 **13/15** 検出・誤棄却 2/56 | §6 |
| 日報をコーパスに入れる | +0.030 [-0.012,+0.083] | 向きは同じだが、#84 の再測定で有意でなくなった |

### 効かなかった（測って捨てたもの。ここが本文の価値）

| 手法 | 効果 | なぜ効かないか |
|---|---|---|
| Dense + BM25 の RRF（**旧 C4 の構成**） | **-0.173** [-0.277,-0.077] | §3 |
| BM25 単独 | -0.342 | 症状語しか出ないクエリと語彙が合わない |
| クロスエンコーダのリランカー（bge-reranker-v2-m3） | -0.033 | §5。+0.63s/クエリ |
| クロスエンコーダのリランカー（Qwen3-Reranker-0.6B） | -0.077 | 同上。+0.69s/クエリ |
| HyDE（仮想文書で引く） | **-0.128** [-0.244,-0.015] | 段Aには効く（acc@1 0.554→0.661）が、人への集約では有意に悪化 |
| Query2doc（クエリ+専門語） | +0.027 [-0.068,+0.125] | 唯一プラスに振れたクエリ拡張だが有意でない |
| LLM listwise リランク（RankGPT型） | -0.006 | 弱い候補集合には効くが、良い候補集合では動かない |
| 自己整合性（5サンプル多数決） | ±0.00 | 段A acc@1 は 0.821→0.839 だが段Cは同値。5倍のコストに見合わない |
| 埋め込み2本のアンサンブル | -0.048 | |
| 人物中心の索引（Balog Model 1） | -0.036 | Model 2 優位という Balog 2006 の報告と同じ向き |
| 集約関数の変更（max / top3和 / 件数正規化） | -0.060 〜 ±0.000 | 現行の順位重み和が既に妥当 |
| 案件共起グラフでの伝播（APPNP型） | -0.006 〜 -0.033 | **45件のときの +0.011 は noise だった**（訂正） |
...
> **フィルタを揃えても結論は不変**（等重み -0.173／BM25 0.2 で -0.015）。日本語は機能語の
> オーバーラップが広く、production でも BM25 の弱い順位が等重み RRF を汚すため。

> **#84 で評価クエリ8件の文面を変えたため 2026-08-23 に測り直した。**
> 基準（Dense のみ）は 0.607 で、#68 の値と一致した。
> **以前あった「§1・§4 の 0.601 とのスナップショット差」は解消し、全節で 0.607 に揃っている。**

| 構成 | R@3 | Δ | 95%CI |
|---|---|---|---|
| Dense のみ | 0.607 | — | |
| Dense + BM25 RRF（等重み＝旧C4） | 0.435 | **-0.173** | [-0.277,-0.077] |
| BM25 重み 0.5 | 0.503 | -0.104 | [-0.199,-0.015] |
| BM25 重み 0.2 | 0.592 | -0.015 | [-0.083,+0.048] |
| BM25 重み 0.1 | 0.595 | -0.012 | [-0.065,+0.030] |
| BM25 のみ | 0.265 | -0.342 | [-0.455,-0.232] |
...
|---|---|---|---|---|
| 基準（Dense 集約） | 0.607 | — | | |
| 検索由来トピック → 構造化 | 0.568 | -0.039 | [-0.116,+0.039] | 0.17 |
| LLM（文脈なし）→ 構造化 | 0.682 | +0.074 | [-0.048,+0.190] | 0.88 |
| **LLM（文脈つき）→ 構造化** | **0.759** | +0.152 | [+0.054,+0.247] | 1.00 |
| LLM（「該当なし」選択肢つき）→ 構造化 | 0.771 | +0.164 | [+0.065,+0.262] | 1.00 |
| LLM（文脈つき）+ 検索由来(専門語) → 構造化 | 0.747 | +0.140 | [+0.042,+0.241] | 1.00 |
| ＋ listwise リランクの RRF | 0.753 | +0.146 | [+0.048,+0.241] | 1.00 |
...
|---|---|---|---|---|
| 主 gold（projects+daily 由来、n=56） | 全証拠 | 0.759 | +0.152 | [+0.054,+0.247] |
| 主 gold | **案件を使わない**（経路が交わらない） | 0.699 | **+0.092** | [+0.003,+0.191] ※ |
| 主 gold | 回答を使わない | 0.509 | -0.098 | [-0.223,+0.033] ※ |
...
> ⚠️ **※印の 95%CI は #84 で測り直していない（基準 0.601 時代のもの）。**
> Δ 列は「絶対値 − 0.607」で計算し直したが、**区間はペア・ブートストラップの出力**なので
> 基準の差だけ平行移動しても求まらない。この表を出したスクリプトが残っていないため再計算できず、
> **旧基準で測った区間をそのまま置いてある**。Δ と区間で基準が食い違っている点に注意。
>
> ⚠️ **「第2の正解（n=45）」3行の Δ は #84 で再計算していない。** この列の基準は
> n=45 部分集合ごとの Dense 単体スコアで、その値を保存したファイルが見当たらず、
> 再生成できたか確認できなかった。**#84 で文面を変えた8件のうち5件（id 11, 14, 16, 18, 21）は
> この n=45 に入る**ので、基準がわずかに動いている可能性がある。
> R@3 の絶対値（0.852 / 0.867 / 0.493）はトピック媒介の出力で、§8-3 のとおり
> LLM 分類を測り直していないため据え置き。**主 gold の3行は 0.607 基準で数え直し済み。**

読み方は2つある。

1. **循環ではなさそうだが、有意性は #84 後に確かめ直せていない。** 主 gold（projects+daily 由来）
   に対して、**案件をまったく見ない**スコアラーでも **+0.092** と基準を上回る。向きは変わっていない。
   ただし **95%CI（※印）は #84 前の基準 0.601 で測ったもので、測り直していない**（下記）。
   旧基準では [+0.003,+0.191] とかろうじて 0 を外していたが、**基準が 0.607 に上がった今も
...
1. **56件しかない。** ±0.09 未満の差は分離できない。本書で「効いた」と書いたのは
   分割検証で +0.118 が残った**トピック媒介という構成の選択**であって、
   その中の変種（+0.140 〜 +0.164）の優劣ではない
2. **合成データである。** 特に BM25 の不利は、症状語で書くという評価セットの設計が
   増幅している可能性がある
3. **#84 の再測定は LLM 分類を測り直していない。** #84 で文面を変えた8件
   （id 11〜16, 18, 21）について、`ablation/llm_topic*.json` に保存してある
   **LLM のトピック予測は旧文面に対する出力のまま**である。埋め込みと構造化スコアラーは
   測り直したが、段A の分類だけは vLLM サーバを立て直す必要があるため未実施。
   変えたのは制約の言い回し（および参照先の言及）で**相談内容そのものは同じ**なので
   予測トピックは変わらないと考えているが、**測っていない**。→ 段A の acc と、
   それを入力に使う段C の値は、この8件について厳密には旧文面基準
