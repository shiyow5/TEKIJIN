...
| 手法 | 効果 | なぜ効かないか |
|---|---|---|
| Dense + BM25 の RRF（**現行 C4 の構成**） | **-0.170** [-0.271,-0.071] | §3 |
...
> **フィルタを揃えても結論は不変**（等重み -0.170／BM25 0.2 で -0.009）。日本語は機能語の
> オーバーラップが広く、production でも BM25 の弱い順位が等重み RRF を汚すため。

下表の Dense のみ 0.607 は #68 の DGX 再実行（production 整合ハーネス）で測ったもので、
§1・§4 の 0.601 はそれ以前の実行スナップショット。Dense 経路は BM25 フィルタに触れないが、
評価スナップショットの差で微小に異なる。

| 構成 | R@3 | Δ | 95%CI |
|---|---|---|---|
| Dense のみ | 0.607 | — | |
| Dense + BM25 RRF（等重み＝旧C4） | 0.438 | **-0.170** | [-0.271,-0.071] |
