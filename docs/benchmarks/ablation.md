...
| | 層2 R@3 | MRR |
|---|---|---|
| 現行（Dense 検索 → チャンクを人へ集約） | 0.592 | 0.710 |
| **トピック媒介（LLM分類 → 構造化スコアラー）** | **0.747 – 0.771** | 0.836 |

**分割検証（56件を半分に割り、片方で構成を選び、もう片方で測る／200回）で
+0.130（中央値 +0.131、5–95%tile: +0.035 – +0.226、96.5% の分割で改善）。**
全件で最良構成を選んだときの +0.179 は多重比較で楽観的なので、採用判断はこの +0.130 で行う。
...
| 手法 | 効果 | 備考 |
|---|---|---|
| **トピック媒介の構造化スコアリング** | **+0.167** [+0.065,+0.265] | 下記 §4 |
...
| 手法 | 効果 | なぜ効かないか |
|---|---|---|
| Dense + BM25 の RRF（**旧 C4 の構成**） | **-0.161** [-0.265,-0.068] | §3 |
| BM25 単独 | -0.339 | 症状語しか出ないクエリと語彙が合わない |
| クロスエンコーダのリランカー（bge-reranker-v2-m3） | -0.018 | §5。+0.63s/クエリ |
| クロスエンコーダのリランカー（Qwen3-Reranker-0.6B） | -0.063 | 同上。+0.69s/クエリ |
| HyDE（仮想文書で引く） | **-0.113** [-0.232,+0.009] | 段Aには効く（acc@1 0.554→0.661）が、人への集約では落ちる（#84 後は区間が 0 を含む） |
| Query2doc（クエリ+専門語） | +0.042 [-0.057,+0.146] | 唯一プラスに振れたクエリ拡張だが有意でない |
| LLM listwise リランク（RankGPT型） | -0.006 | 弱い候補集合には効くが、良い候補集合では動かない |
| 自己整合性（5サンプル多数決） | ±0.00 | 段A acc@1 は 0.821→0.839 だが段Cは同値。5倍のコストに見合わない |
| 埋め込み2本のアンサンブル | -0.051 | |
| 人物中心の索引（Balog Model 1） | -0.021 | Model 2 優位という Balog 2006 の報告と同じ向き |
| 集約関数の変更（max / top3和 / 件数正規化） | -0.054 〜 +0.003 | 現行の順位重み和が既に妥当 |
| 案件共起グラフでの伝播（APPNP型） | -0.006 〜 -0.021 | **45件のときの +0.011 は noise だった**（訂正） |
...
> **フィルタを揃えても結論は不変**（等重み -0.161／BM25 0.2 で -0.009）。日本語は機能語の
> オーバーラップが広く、production でも BM25 の弱い順位が等重み RRF を汚すため。

> **#84 で評価クエリ8件の文面を変えたため 2026-08-23 に測り直した。**
> 基準（Dense のみ）は **0.592**（#84 の生成器修正後の再測定）。
> **以前あった「§1・§4 とのスナップショット差」は解消し、全節で同じ基準に揃っている。**

| 構成 | R@3 | Δ | 95%CI |
|---|---|---|---|
| Dense のみ | 0.592 | — | |
| Dense + BM25 RRF（等重み＝旧C4） | 0.432 | **-0.161** | [-0.265,-0.068] |
| BM25 重み 0.5 | 0.485 | -0.107 | [-0.199,-0.021] |
| BM25 重み 0.2 | 0.583 | -0.009 | [-0.074,+0.054] |
| BM25 重み 0.1 | 0.586 | -0.006 | [-0.060,+0.036] |
| BM25 のみ | 0.253 | -0.339 | [-0.452,-0.229] |
...
|---|---|---|---|---|
| 基準（Dense 集約） | 0.592 | — | | |
| 検索由来トピック → 構造化 | 0.551 | -0.042 | [-0.122,+0.036] | 0.16 |
| LLM（文脈なし）→ 構造化 | 0.682 | +0.089 | [-0.036,+0.211] | 0.92 |
| **LLM（文脈つき）→ 構造化** | **0.759** | +0.167 | [+0.065,+0.265] | 1.00 |
| LLM（「該当なし」選択肢つき）→ 構造化 | 0.771 | +0.179 | [+0.077,+0.277] | 1.00 |
| LLM（文脈つき）+ 検索由来(専門語) → 構造化 | 0.747 | +0.155 | [+0.054,+0.256] | 1.00 |
| ＋ listwise リランクの RRF | 0.753 | +0.161 | [+0.062,+0.259] | 1.00 |
...
|---|---|---|---|---|
| 主 gold（projects+daily 由来、n=56） | 全証拠 | 0.759 | +0.167 | [+0.065,+0.265] |
| 主 gold | **案件を使わない**（経路が交わらない） | 0.699 | **+0.107** | [+0.003,+0.191] ※ |
| 主 gold | 回答を使わない | 0.509 | -0.083 | [-0.223,+0.033] ※ |
...
> ⚠️ **※印の 95%CI は測り直していない（基準 0.601 時代のもの）。**
> Δ 列は「絶対値 − 基準」で計算し直したが、**区間はペア・ブートストラップの出力**なので
...
1. **56件しかない。** ±0.09 未満の差は分離できない。本書で「効いた」と書いたのは
   分割検証で +0.130 が残った**トピック媒介という構成の選択**であって、
   その中の変種（+0.155 〜 +0.179）の優劣ではない
