...
3. **#84 の再測定は LLM 分類を測り直していない。** #84 で文面を変えた8件
   （**id 11〜18**。id 21 は生成器修正で元の文面に戻ったので対象外）について、`ablation/llm_topic*.json` に保存してある
