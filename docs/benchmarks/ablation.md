...
| | 層2 R@3 | MRR |
|---|---|---|
| 現行（Dense 検索 → チャンクを人へ集約） | 0.562 | 0.639 |
| **トピック媒介（LLM分類 → 構造化スコアラー）** | **0.705 – 0.735** | 0.792 |

**分割検証（56件を半分に割り、片方で構成を選び、もう片方で測る／200回）で
+0.129（中央値 +0.131、5–95%tile: +0.041 – +0.208、97.5% の分割で改善）。**
全件で最良構成を選んだときの +0.173 は多重比較で楽観的なので、採用判断はこの +0.129 で行う。

**律速はトピック推定だった。** 検索結果からトピックを推定すると acc@1=0.500 だが、
...
| 手法 | 効果 | 備考 |
|---|---|---|
| **トピック媒介の構造化スコアリング** | **+0.161** [+0.071,+0.253] | 下記 §4 |
| 検索文脈つきの LLM 分類（RAG型分類） | 段A acc@1 0.714 → **0.821** | +0.06s |
| 棄却は「証拠の十分性」を別段で判定 | L4 **13/15** 検出・誤棄却 2/56 | §6 |
| 日報をコーパスに入れる | +0.033 [-0.012,+0.089] | 向きは同じだが、#84 の再測定で有意でなくなった |

### 効かなかった（測って捨てたもの。ここが本文の価値）

| 手法 | 効果 | なぜ効かないか |
|---|---|---|
| Dense + BM25 の RRF（**旧 C4 の構成**） | **-0.128** [-0.217,-0.042] | §3 |
| BM25 単独 | -0.292 | 症状語しか出ないクエリと語彙が合わない |
| クロスエンコーダのリランカー（bge-reranker-v2-m3） | +0.009 | §5。+0.63s/クエリ。**#158 で符号が反転したが有意でない** |
| クロスエンコーダのリランカー（Qwen3-Reranker-0.6B） | -0.065 | 同上。+0.69s/クエリ |
| HyDE（仮想文書で引く） | **-0.104** [-0.217,+0.012] | 段Aには効く（acc@1 0.500→0.661）が、人への集約では落ちる（区間は 0 を含む） |
| Query2doc（クエリ+専門語） | +0.036 [-0.065,+0.146] | 唯一プラスに振れたクエリ拡張だが有意でない |
| LLM listwise リランク（RankGPT型） | -0.009 | 弱い候補集合には効くが、良い候補集合では動かない（§4 の 0.714 を「LLM 文脈つき」0.723 と比べた値） |
| 自己整合性（5サンプル多数決） | ±0.00 | 段A acc@1 は 0.821→0.839 だが段Cは同値。5倍のコストに見合わない |
| 埋め込み2本のアンサンブル | -0.006 | |
| 人物中心の索引（Balog Model 1） | ±0.000 | **#158 で差が消えた**。Balog 2006 の Model 2 優位は再現しなくなった |
| 集約関数の変更（max / top3和 / 件数正規化） | -0.057 〜 -0.006 | 現行の順位重み和が既に妥当 |
| 案件共起グラフでの伝播（APPNP型） | +0.006 〜 -0.030 | **45件のときの +0.011 は noise だった**（訂正） |
...
|---|---|---|---|
| Dense のみ | 0.562 | — |  |
| Dense + BM25 RRF（等重み＝旧C4） | 0.435 | **-0.128** | [-0.217,-0.042] |
| BM25 重み 0.5 | 0.500 | -0.062 | [-0.155,+0.030] |
| BM25 重み 0.2 | 0.527 | -0.036 | [-0.107,+0.030] |
| BM25 重み 0.1 | 0.539 | -0.024 | [-0.080,+0.024] |
| BM25 のみ | 0.271 | -0.292 | [-0.405,-0.176] |
<!-- /gen:bm25 -->
...
| LLM(該当なし選択肢つき) | 0.786 | 0.911 |
| LLM(自己整合性5回) | 0.839 | 0.911 |
| LLM(文脈つき)+検索由来(専門語) | 0.804 | 0.893 |
<!-- /gen:stage_a -->
...
|---|---|---|---|---|
| 基準（Dense 集約） | 0.562 | — |  |  |
| 検索由来のみ → 構造化 | 0.464 | -0.098 | [-0.185,-0.012] | 0.01 |
| LLM(文脈なし) → 構造化 | 0.652 | +0.089 | [-0.033,+0.205] | 0.93 |
| **LLM(検索文脈つき) → 構造化** | **0.723** | +0.161 | [+0.071,+0.253] | 1.00 |
| LLM(該当なし選択肢つき) → 構造化 | 0.735 | +0.173 | [+0.080,+0.265] | 1.00 |
| LLM(自己整合性5回) → 構造化 | 0.723 | +0.161 | [+0.071,+0.253] | 1.00 |
| LLM(文脈つき)+検索由来 → 構造化 | 0.625 | +0.063 | [-0.021,+0.149] | 0.92 |
| LLM(文脈つき)+検索由来(専門語) → 構造化 | 0.705 | +0.143 | [+0.042,+0.244] | 1.00 |
| 検索由来(クエリ+HyDE) → 構造化 | 0.577 | +0.015 | [-0.095,+0.122] | 0.61 |
| LLM(文脈つき)+検索由来 → 構造化 → listwiseリランクRRF | 0.714 | +0.152 | [+0.057,+0.244] | 1.00 |
<!-- /gen:stage_c -->
...
|---|---|---|---|---|
| 主 gold（projects+daily 由来、n=56） | 全証拠 | 0.723 | +0.161 | [+0.071,+0.253] |
...
> LLM 分類を測り直していないため据え置き。**主 gold の1行目のみ 0.562 基準で測り直し済み（他2行は下記）。**
...
1. **56件しかない。** ±0.09 未満の差は分離できない。本書で「効いた」と書いたのは
   分割検証で +0.129 が残った**トピック媒介という構成の選択**であって、
   その中の変種（+0.143 〜 +0.173）の優劣ではない
