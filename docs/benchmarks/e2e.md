...
> ⚠️ **#84 で評価クエリ8件（id 11〜16, 18, 21）の文面を変えたため、この節の数値は再測定待ち。**
> 変えたのは文面だけで **id・gold ラベル・難易度の内訳（採点対象56件 / gold トピックあり52件）は不変**
> なので、**分母は動かない**。動きうるのは検索を通る行だけで、
> **候補を全社員にする行（0.836 / C1 実トピックの 0.750）は影響を受けない**
> （`rank(q, all_ids)` は `res` も `route` も読まないため）。→ [robustness.md](robustness.md) §4
...
|---|---|---|---|---|---|---|---|
| **現状（そのまま）** | **0.673** (0.724) | 0.661 | 0.705 | 0.500 | 0.731 | 0.633 | 0.140 |
| 経路の pin を外す（C4 の候補10名） | **0.792** (0.853) | 0.786 | 0.830 | 0.967 | 0.787 | 0.633 | 0.732 |
| **候補を全社員にする**（[scorer.md](scorer.md) §3 / #87） | **0.836** (0.901) | 0.804 | 0.866 | 0.967 | 0.819 | 0.767 | 0.836 |

- **#120 の閾値較正で 0.140 → 0.673。** #103（全件 `prior_answer` で候補が1名に固定される）は解消した
...
- C4 で候補を10名に絞ることの損失は **0.836 − 0.792 = 0.044**（旧構成では 0.104）

### 0.2 「現状そのまま」の 0.673 は、分母に構造上0点の11件を含む
...
到達可能な上限は **45/56 = 0.804**。実測 0.673 は、その 45件のうち **0.837 相当**を取っている。
...
| `people_confidence` | 0.052 | 0.227 | 0.453 | `PERSON_WEAK_SIM = 0.40` |
...
| C1 の実トピック + 現状の経路 | **0.595** | 0.622 | 0.146 |
