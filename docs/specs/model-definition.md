...
対象プロダクト: TEKIJIN。**暗黙知を「人に聞く」履歴として蓄積し、形式知へ変換し続ける**基盤。
社内の「これ誰に聞けばいいんだろう」を AI が正しい人へ正しい形で取り次ぎ、その一次情報を
ナレッジとして蓄える（取次ぎは蓄積を回す手段）。~~**答えの出所は必ず人。**~~（⚠ 2026-08-25 改訂中／下記「方針転換」参照）

> **⚠ 方針転換（2026-08-25・#291 / #292）**: 「**答えの出所は必ず人**」を廃す。メイン機能は
> **AIチャットボット**で、①既存の社内データ・過去の回答にあるもの → **AI が出典（ソースのリンク）付きで
> 自己回答**（人に取り次がない）、②データが無い/暗黙的なもの → **適切な社員に取り次ぐ**、③②で見つかった
> 暗黙知は**チャット内容から形式知へ変換して蓄積**する。これに伴い **C5 は「自己回答（データ十分）vs 取次ぎ」
> の判定に再定義**し、**回答生成コンポーネント C7'（self-answer）を追加**する（本ドキュメントの全面改訂は #292。
> 実装は #291 で段階的に。まず self-answer コンポーネントを dormant で追加済み＝`self_answer_enabled=False`）。
> **評価は recall 中心**で見る（メンター方針・#297）。回答時は**根拠のソースリンクをチャット内に含める**。
...
| C7' | 自己回答生成（#291・**未配線/dormant**） | LLM（構造化出力） | ノード（LLM、`self_answer_enabled` で gate） | 質問＋検索根拠（過去QA/文書, 出典ID付き） → 出典付き回答 or 「答えられない」 |
