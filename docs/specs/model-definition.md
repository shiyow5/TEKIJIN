...
> 実装は #291 で段階的に。part1=コンポーネント追加、part2=グラフ配線まで完了。**既定OFF**
> `self_answer_enabled=False`（有効化は part3 の recall中心検証の後）。データ由来経路で発火し
> grounded なら `self_answered` 終端で出典付き回答、不足なら元の取次ぎ経路へフォールバック）。
