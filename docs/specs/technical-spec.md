...
| 統合 | **重み付き RRF（Reciprocal Rank Fusion, k=60）** | スコアのスケールが違う2系統を順位だけで統合。`score(d)=Σ_r w_r/(k+rank_r(d))`。dense=1.0・**BM25=0.2（#68）**：等重みは症状語クエリで BM25 の弱い順位が dense を汚し層2 R@3 -0.200 だったため。適応重みは #114 |
