...
| 統合 | **RRF（Reciprocal Rank Fusion, k=60）** | スコアのスケールが違う2系統を、順位だけで統合できる。正規化のチューニングが不要 |
...
| 項目 | 採用 | 根拠 |
| --- | --- | --- |
| 第一候補 | **日本語特化の埋め込みモデル**（例: `cl-nagoya/ruri-v3-310m`） | 日本語の意味検索で多言語モデルより有利な傾向。軽量でローカル実行できる |
| 第二候補 | **`intfloat/multilingual-e5-large`** | 実績が多く、情報が豊富。`query:` / `passage:` のプレフィックスが必要な点に注意 |
| 実行 | sentence-transformers（ローカル） | 外部API不要。ネットワーク非依存の要件を満たす |

> **両方を評価セットで測って選ぶこと。** 「有名だから」で選ばない。
> **測って選んだという事実そのものが、技術完成度の評価材料になる。**
> モデル名は執筆時点の候補なので、着手時に最新のJMTEB等のベンチマークを確認して決めること。
